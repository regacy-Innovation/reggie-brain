...
The developer role may deliver changes only to the selected project's registered `development_push_branch`. When repository protection requires a pull request, it may push its isolated topic branch solely as the source of a pull request targeting that registered development branch; it must not target any other branch. Direct pushes and merges remain limited to the registered development branch. It records `development` as the deployment environment and never initiates, approves, or carries out a production deployment. A `main` development branch is allowed only when the selected registry entry sets `main_is_development_only: true`.
