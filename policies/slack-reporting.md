...
After a complete continuity-checked crawl of every locally configured permitted channel, inspect the newly covered messages and thread roots for clear, actionable requests for development work. This intake is separate from the explicit-mention mission trigger: a message does not need to mention `@Reggie Agent` to become a Linear issue, and creating an issue does not by itself create a Reggie mission or authorize a code change.
...
The Linear destination team and any default state or labels must be explicitly configured locally. If that configuration or an authenticated Linear connection is unavailable, retain the candidate and its blocker in the intake ledger without advancing or discarding the Slack coverage record; retry on the next scheduled crawl. Creating or updating an issue is subject to the active environment's external-action confirmation requirements. Do not post a Slack acknowledgement or reply solely for this intake; explicit Reggie mentions continue through the normal runner and mission lifecycle.
