...
> **Non-negotiable intake invariant:** completing a channel crawl also requires completing Linear intake for that newly covered interval. For each clear, actionable development request, Reggie must either create its deduplicated Linear issue or retain a durable intake-ledger entry with the concrete creation blocker. Never treat a message as handled merely because it lacks an `@Reggie Agent` mention, already has a Slack reaction, is directed to another teammate, or appears in a thread. The only permitted exception is an incomplete crawl, which must remain explicitly recorded as incomplete and retried from its saved cursor.
...
When a Slack request or its ordered follow-up messages contain multiple independently deliverable outcomes, create one issue for each outcome instead of merging them into a broad card. Retain the exact source permalink for each issue, and keep shared context concise in each description. Use one combined issue only when the steps are inseparable parts of one deliverable or the requester explicitly asks for a single card.
...
The Linear destination team, any default state or labels, and an assignee-routing map for development work must be explicitly configured locally. New intake issues must be created in **ToDo** by default. A requester-specified classification or state takes precedence; do not infer a different classification and do not default to Backlog. For every created intake issue, assign the owner selected by that map; use the Slack request's explicit owner when present, otherwise use the configured component or fallback owner. Do not invent an assignee. If the configuration, the selected owner, or an authenticated Linear connection is unavailable, retain the candidate and its blocker in the intake ledger without advancing or discarding the Slack coverage record; retry on the next scheduled crawl.
