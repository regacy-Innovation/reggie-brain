...
The Linear destination team, any default state or labels, and an assignee-routing map for development work must be explicitly configured locally. For every created intake issue, assign the owner selected by that map; use the Slack request's explicit owner when present, otherwise use the configured component or fallback owner. Do not invent an assignee. If the configuration, the selected owner, or an authenticated Linear connection is unavailable, retain the candidate and its blocker in the intake ledger without advancing or discarding the Slack coverage record; retry on the next scheduled crawl. Creating, assigning, or updating an issue is subject to the active environment's external-action confirmation requirements. Do not post a Slack acknowledgement or reply solely for this intake; explicit Reggie mentions continue through the normal runner and mission lifecycle.
