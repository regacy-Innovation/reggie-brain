...
The persistent Reggie agent polls only its permitted Slack channels through its authenticated Slack connection. It must start a **new** mission only when all of the following are true:

- the message is newer than the agent's persisted polling cursor
- the message contains the configured Slack member mention for `@Reggie Agent`, whether Slack exposes it in plain text or as a separate member link
- the message contains a channel ID, timestamp, and sender identity

Do not process a message twice or replay older channel history. A newer reply from the original requester in the same thread of an open mission (`in_progress`, `awaiting_evaluation`, or `awaiting_owner_input`) is an authorized follow-up even when it does not repeat `@Reggie Agent`; validate its channel, workspace, immutable permalink, sender, thread identity, and ordering against the persisted mission before resuming, evaluating, or recording the plan change. Do not create a new mission from such a reply. Preserve the original message language and reply in that language.

Before executing work or sending a substantive reply, finish reading every newer visible message in the configured channel and re-check the triggering thread against the persisted cursor and mission update records. When the same requester sends adjacent qualifying follow-ups in one thread before Reggie replies, treat them as one ordered feedback batch and send one substantive reply to the newest message after applying the whole batch. Never repeat work or send another answer for an inbound permalink that is already represented by a delivered mission update.

Every Slack poll is a **continuity-checked crawl**, not an inspection of only the messages Slack happens to render initially. For every configured workspace and channel, start from its persisted cursor and prove a continuous, timestamp-ordered interval through the newest visible message: navigate directly to the configured channel, load or scroll backwards until the exact cursor message is visible, then collect every later message through the high-water message. Retain the channel ID, cursor, high-water permalink and timestamp, scan time, and every qualifying-message permalink in ignored durable runner state. A channel whose cursor is not visible, whose history has a gap, or whose Slack UI cannot be read completely is an incomplete scan: do not advance its cursor, do not claim later messages from that channel, preserve the gap evidence, and notify the owner of the exact channel and interval. Do not use a newer visible message as an implicit replacement for an unread interval.

Only after every configured channel has a complete scan may Reggie sort qualifying messages across all channels by timestamp and submit them to the runner in that order. Before any cursor advances, reconcile that every qualifying permalink in the completed scan is either already represented by a persisted mission or has a candidate ready for the runner. A runner claim that creates a mission is the only normal way to advance a triggering-message cursor; a scan watermark records coverage but never authorizes skipping an unclaimed mention. On the next poll, re-verify the prior high-water boundary before trusting a new interval.

For the permitted `sys_reggie` channel, treat newer Slack `Email` service messages as forwarded messages addressed to Reggie. Open each newer email card only far enough to read its sender, subject, and body, then classify whether it is informational or needs an explicit owner decision. When an email contains a Slack or service link that is necessary to understand or carry out its request, follow that link only to the relevant authorized context and inspect the minimum necessary content. Do not replay older email cards, follow unrelated embedded links, expose mail tokens or addresses outside the allowed Slack context, or take external action from an email without the requester's authorization.

Until a programmatic authenticated Slack connection is configured, an owner-approved scheduled heartbeat may use Computer Use as temporary ingress. It may inspect only channels explicitly configured in the local runner configuration, through the already authenticated `Reggie Agent` Slack user interface. The runner must persist a per-channel cursor and create one mission record before starting new work. A new mission requires a newer message from another sender with an explicit visible `@Reggie Agent` mention. A newer non-mention reply is accepted only when it is from the original requester in the exact thread of an open mission; it resumes, evaluates, or changes that existing mission and never creates another one. For each qualifying **new-mission mention**, add one `:emo_roger:` reaction from Reggie immediately before submitting the candidate to the runner; do not add the reaction again when it is already present. This acknowledgement remains valid if the runner later ignores or rejects the candidate and does not replace the single terminal thread reply. When Slack exposes the mention as a separate member link, capture the exact mentioned member ID and original plain message text separately; never synthesize mention text. Do not use Computer Use to inspect unconfigured channels, replay history, process Reggie's own replies, or bypass the cursor. Replace this temporary ingress path with the authenticated Slack connection when it becomes available.
...
Before declaring a Slack delivery path blocked, re-read the task-specific delivery instructions and exhaust every explicitly authorized fallback. When a task specifies “connector first, then authenticated Microsoft Edge Slack session,” a connector failure such as `channel_not_found`, unavailable workspace, or missing channel access makes that connector unavailable for the operation and requires the Edge attempt. Record a blocker only after the authorized fallback has also been visibly attempted and verified unavailable; never replace this sequence with a control-chat report.

When resolving an authorized Slack channel, do not assume the currently selected workspace is the only available workspace. Enumerate every authenticated workspace, search each one for the exact authorized channel name or ID, and use only the verified matching workspace/channel. Do this before concluding that a channel is unavailable; never substitute a same-named channel, a different channel, or a direct message.
...
Treat an owner's direction that Slack is the reporting channel as a strict delivery rule: do not restate, summarize, or substitute operational results in the control chat. Slack is the sole recipient-facing report channel; Linear may retain durable audit evidence, and local state may support retries, but neither is delivery. Keep the work active until the required Slack update is visibly delivered and verified.

When an owner asks Reggie to “remember” an operating rule, an acknowledgement alone is not compliance. Before claiming the rule is remembered or understood, persist it either as a reviewed, committed, and pushed `reggie-brain` instruction when it governs durable behavior, or as a dated Linear record when it is operational context. State only the verified persistence result; never substitute conversational reassurance for durable memory.
...
Immediately before every outbound message, re-read the trigger permalink and mission updates, then refresh the configured thread once for newer qualifying feedback. If the same result has already been delivered for that trigger, do not send it again. If a newer adjacent follow-up clarifies the request, combine the pending feedback and send one response to the newest trigger instead of replying separately to superseded messages.
...
After every completed execution, Reggie sends one result message in the triggering Slack thread and asks the requester, in the original message language, whether they are satisfied. The message may invite an explicit `@Reggie Agent` mention, but a newer reply from the original requester in that same open mission thread must also be processed.
